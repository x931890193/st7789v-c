        ...
        for (col = 0; col < bmpInfoHeader.bWidth;) {
            if (bmpInfoHeader.bBitCount == 16) {
                ...
                if (bmpInfoHeader.bInfoSize == 0x38) {
                    ...
                    data = ((((long) ((data >> 5) & 0x1f) * 0X3F) / 0X1F) << 5) + (data & 0x1f) +
                           ((data & 0xEC00) << 1);
                    Paint_SetPixel(col, bmpInfoHeader.bHeight - row - 1, data);
                }
                col++;
                ...
                printf("------");
                printf("col=%3d  ",col);
                printf("row=%3d  ",bmpInfoHeader.bHeight - row - 1);
                printf("rgbRed=%3d rgbGreen=%3d rgbBlue=%3d \n",argb.rgbRed, argb.rgbGreen, argb.rgbBlue);
#endif
                col++;
            }
                //bBitCount<8 format
